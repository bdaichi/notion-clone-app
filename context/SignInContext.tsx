...
const SignInContext = createContext<SignInContextProps>({
    currentUser: null,
    reloadCurrentUserData: () => { throw new Error('Disabled reloadCurrentUserData() on SignInContext') },
});
...
const SignInProvider: FC = ({ children }) => {
    ...
    return (
        <SignInContext.Provider value={{ currentUser: currentUser, reloadCurrentUserData: reloadCurrentUserData, }}>
            {/* initUserの場合はloading */}
            {currentUser == null || (currentUser != null && currentUser.userId != 'init')
                ? children
                : <div className="flex justify-center py-80"><CircularProgress color="primary" /></div>
            }
